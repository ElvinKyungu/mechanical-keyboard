...
<template>
  <div class="min-h-screen bg-zinc-950 flex items-center justify-center overflow-hidden select-none">
    <div 
      ref="keyboardRef"
    ...
    >
      ...
      <div class="flex mb-4 transform-gpu" style="transform-style: preserve-3d;">
        ...
        <KeyButton v-for="i in 12" :key="`f${i}`" :code="`Digit${i}`" class="ml-4">
          F{{ i }}
        </KeyButton>
        <KeyButton code="Backspace" size="large" class="ml-4 bg-zinc-700 hover:bg-zinc-600 px-6">
          ←
        </KeyButton>
        <KeyButton color="red" macro="red" class="ml-4 bg-red-600 hover:bg-red-700" @animate="handleMacroAnimation" />
      </div>

      <!-- Row 2: Top Letter Row -->
      <div class="flex mb-4 transform-gpu" style="transform-style: preserve-3d;">
        <KeyButton code="Tab" size="medium" class="bg-zinc-700 hover:bg-zinc-600 px-6">Tab</KeyButton>
        <KeyButton v-for="key in qwertyRow" 
          :key="key" 
          :code="`Key${key}`" 
          class="ml-4"
        >
        ...
        <KeyButton code="Backslash" size="medium" class="ml-4 bg-zinc-700 hover:bg-zinc-600 px-6">//</KeyButton>
        <KeyButton color="yellow" macro="yellow" class="ml-4 bg-yellow-600 hover:bg-yellow-700" @animate="handleMacroAnimation" />
      </div>

      <!-- Row 3: Home Row -->
      <div class="flex mb-4 transform-gpu" style="transform-style: preserve-3d;">
        <KeyButton code="CapsLock" size="large" class="bg-zinc-700 hover:bg-zinc-600 px-8">Caps Lock</KeyButton>
        <KeyButton v-for="key in homeRow" 
          :key="key" 
          :code="`Key${key}`" 
          class="ml-4"
        >
        ...
        <KeyButton color="green" macro="green" class="ml-4 bg-green-600 hover:bg-green-700" @animate="handleMacroAnimation" />
      </div>
      ...
      <div class="flex mb-4 transform-gpu" style="transform-style: preserve-3d;">
        ...
        <KeyButton v-for="key in bottomRow" 
          :key="key" 
          :code="`Key${key}`" 
          class="ml-4"
        >
        ...
        <KeyButton color="blue" macro="blue" class="ml-4 bg-blue-600 hover:bg-blue-700" @animate="handleMacroAnimation" />
      </div>
...
const initializeKeyPositions = async () => {
  ...
  const allKeyElements = keyboardRef.value?.querySelectorAll('.key-button') as NodeListOf<HTMLElement>
  ...
  macroColors.forEach(color => {
    const macroKey = keyArray.find(el => 
...
const handleMacroAnimation = (macro: string) => {
  ...
  const capsLockPressed = capsLockState ? keysDown.value.has(capsLockState.index) : false
  
  if (!capsLockPressed && ['Space', 'ShiftLeft'].some(key => macro.includes(key))) {
    return
  }
...
.key-button.small { width: calc(50px * 1.33); }
.key-button.medium { width: calc(50px * 1.65); }
.key-button.large { width: calc(50px * 1.98); }
.key-button.xlarge { width: calc(50px * 2.62); }
.key-button.spacebar { width: calc(50px * 7.8); }
